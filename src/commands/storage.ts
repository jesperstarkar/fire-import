...
export default async function storage() {
  ...
  await execute(
    `gsutil -m cp -r gs://${projectId}.firebasestorage.app ${firebaseStorageBlobsPath}`,
    function (output: any) {
      return;
    }
  );
  ...
  let Bucket_folder = path.resolve(
    `${firebaseStorageBlobsPath}${projectId}.firebasestorage.app`
  );
  ...
  for (let filepath of files) {
    ...
    fs.ensureDirSync(
      `${firebaseStorageMetadataPath}${projectId}.firebasestorage.app/` +
